...
import com.facomp.pethub.tutelado.repository.TuteladoRepository;
...
public class TuteladoService {
    ...
    public TuteladoService(TuteladoRepository tuteladoRepository, TuteladoMapper tuteladoMapper, TutorRepository tutorRepository) {
        this.tuteladoRepository = tuteladoRepository;
    ...
    public TuteladoResponse buscarPorId(Long id) {
        Tutelado tutelado = tuteladoRepository.findById(id)
    ...
    public TuteladoResponse criar(TuteladoRequest tuteladoDto) {
        Tutelado tutelado = tuteladoMapper.mapToEntity(tuteladoDto);
        validaTutor(tutelado);
        tutelado = tuteladoRepository.save(tutelado);
        return tuteladoMapper.mapToDto(tutelado);
    }

    private void validaTutor(Tutelado tutelado) {
        if (!tutorRepository.existsById(tutelado.getTutor().getId())) {
            throw new RegisterNotFoundException("Tutor não encontrado!");
        }
        tutelado.setTutor(tutorRepository.getReferenceById(tutelado.getTutor().getId()));
    }
